...
/**
 * Any agent that wants to use GOAP must implement this interface. It provides information to the GOAP
...
public interface IGoap
{
	 ...
	 */
	Dictionary<string, int> GetWorldState ();

	/**
	 * Give the planner a new goal so it can figure out 
	 * the actions needed to fulfill it.
	 */
	Dictionary<string, int> CreateGoalState ();

	/**
	 * No sequence of actions could be found for the supplied goal.
	 * You will need to try another goal
	 */
	void PlanFailed (Dictionary<string, int> failedGoal);

	/**
	 * A plan was found for the supplied goal.
	 * These are the actions the Agent will perform, in order.
	 */
	void PlanFound (Dictionary<string, int> goal, Queue<GoapAction> actions);
	 ...
	 */
	bool MoveAgent(GoapAction currentAction);
