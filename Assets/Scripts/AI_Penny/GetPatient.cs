...
public class GetPatient : GAction {
    ...
    public override bool PrePerform() {
        ...
        target = GWorld.Instance.GetQueue("Character_Tier1").RemoveResource();
        ...
        if (resource != null) {
            ...
            GWorld.Instance.GetQueue("Character_Tier1").AddResource(target);
            target = null;
            return false;
        }

        //take away one cubicle being available from the world state
        GWorld.Instance.GetWorldStatesHandle().ModifyState("FreeCubicle", -1);
        return true;
    }
